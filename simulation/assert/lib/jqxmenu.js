	...
	a.extend(a.jqx._jqxMenu.prototype, {
		...
		_closeItem : function(l, o, g, c) {
			...
			if(k != null) {
				...
				if(j.data("timer").show != null) {clearTimeout(j.data("timer").show);
				...
				if(h > 0) {
					j.data("timer").hide = setTimeout(function() {n()
					}, h)
				} else {n()
				}
				if(g != undefined && g) {
		...
		_openItem : function(n, q, p) {
			...
			if(j.data("timer").hide != null) {clearTimeout(j.data("timer").hide)
			}
			...
			if(this.autoCloseInterval > 0) {
				if(this.host.data("autoclose") != null && this.host.data("autoclose").close != null) {clearTimeout(this.host.data("autoclose").close)
				}
		...
		_applyOrientation : function(h, c) {
			...
			switch(this.mode) {
				case"horizontal":
					...
					if(f.autoSizeMainItems) {
						if(parseInt(e) > parseInt(this.host.height())) {
						...
						a.each(this.items, function() {
							...
							$element = a(r.element);
							if(r.level == 0 && e > 0) {
								var j = $element.children().length > 0 ? parseInt($element.children().height()) : $element.height();
								var m = f.host.find("ul:first");
								var n = parseInt(m.css("padding-top"));
								var s = parseInt(m.css("margin-top"));
								var p = e - 2 * (s + n);
								var o = parseInt(p) / 2 - j / 2;
								var k = parseInt(o);
								var q = parseInt(o);
								$element.css("padding-top", k);
								$element.css("padding-bottom", q);
								if(parseInt($element.outerHeight()) > p) {
									var l = 1;
									$element.css("padding-top", k - l);
									k = k - l
								}
							}
						})
				...
				if(p.subMenuElement != null) {
					...
					if(!d && !e.clickToOpen) {
						e.addHandler(l, "mouseenter", function() {
							if(e.autoOpen || (p.level > 0 && !e.autoOpen)) {clearTimeout(m.data("timer").hide)
							}
						...
						e.addHandler(l, "mousedown", function() {
							if(!e.autoOpen && p.level == 0) {clearTimeout(m.data("timer").hide);
								if(m != null) {
									m.stop()
								}
								if(!p.isOpen) {
						...
						e.addHandler(l, "mouseleave", function(s) {
							if(e.autoCloseOnMouseLeave) {clearTimeout(m.data("timer").hide);
								var v = a(p.subMenuElement);
								var r = {
									left : parseInt(s.clientX),
									top : parseInt(s.clientY)
								};
								var u = {
						...
						e.addHandler(o, "mouseenter", function() {clearTimeout(m.data("timer").hide)
						});
						...
						e.addHandler(l, "mousedown", function(r) {clearTimeout(m.data("timer").hide);
							if(m != null) {
								m.stop()
							}
