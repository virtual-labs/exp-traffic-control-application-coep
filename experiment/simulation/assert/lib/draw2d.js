...
draw2d.UUID.create = function() {
	...
	return (_20aa() + _20aa() + "-" + _20aa() + "-" + _20aa() + "-" + _20aa() + "-" + _20aa() + _20aa() + _20aa());
};
...
function openwindow(url, width, _16f3) {
	...
	property = "left=" + left + ", top=" + top + ", toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=1,alwaysRaised,width=" + width + ",height=" + _16f3;
...
draw2d.Draggable.prototype.attachEventHandlers = function() {
	...
	this.tempMouseMove = function() {
		...
		var _2be8 = new draw2d.Point(_2be7.clientX - oThis.diffX, _2be7.clientY - oThis.diffY);
		...
		if(oThis.node !== null) {
			...
			var _2bed = oThis.getDropTarget(_2be7.clientX + _2be9 - _2beb, _2be7.clientY + _2bea - _2bec);
			var _2bee = oThis.getCompartment(_2be7.clientX + _2be9 - _2beb, _2be7.clientY + _2bea - _2bec);
			if(draw2d.Drag.currentTarget !== null && _2bed != draw2d.Drag.currentTarget) {
			...
			if(draw2d.Drag.currentCompartment !== null && _2bee !== draw2d.Drag.currentCompartment) {
				var _2bef = new draw2d.DragDropEvent();
				_2bef.initDragDropEvent("figureleave", false, oThis);
				draw2d.Drag.currentCompartment.dispatchEvent(_2bef);
			}
			if(_2bee !== null && _2bee.node != oThis.node && _2bee !== draw2d.Drag.currentCompartment) {
				var _2bef = new draw2d.DragDropEvent();
	...
	oThis.tempMouseUp = function() {
		...
		if(oThis.node !== null) {
			...
			var _2bf6 = oThis.getDropTarget(_2bf1.clientX + _2bf2 - _2bf4, _2bf1.clientY + _2bf3 - _2bf5);
			var _2bf7 = oThis.getCompartment(_2bf1.clientX + _2bf2 - _2bf4, _2bf1.clientY + _2bf3 - _2bf5);
			if(_2bf6 !== null) {
...
draw2d.Color.prototype.getIdealTextColor = function() {
	...
	return (255 - _2597 < _2596) ? new draw2d.Color(0, 0, 0) : new draw2d.Color(255, 255, 255);
};
...
draw2d.SnapToGrid.prototype.snapPoint = function(_2107, _2108, _2109) {
	_2109.x = this.workflow.gridWidthX * Math.floor(((_2108.x + this.workflow.gridWidthX / 2) / this.workflow.gridWidthX));
	_2109.y = this.workflow.gridWidthY * Math.floor(((_2108.y + this.workflow.gridWidthY / 2) / this.workflow.gridWidthY));
	return 0;
};
...
draw2d.SnapToGeometry.prototype.snapRectangle = function(_25cf, _25d0) {
	...
	var _25d3 = this.snapPoint(draw2d.SnapToHelper.NORTH_WEST, _25cf.getTopLeft(), _25d1);
	_25d0.x = _25d1.x;
	_25d0.y = _25d1.y;
	var _25d4 = this.snapPoint(draw2d.SnapToHelper.SOUTH_EAST, _25cf.getBottomRight(), _25d2);
...
draw2d.SnapToGeometry.prototype.populateRowsAndCols = function() {
	...
	for(var i = 0; i < _25d5.getSize(); i++) {
		...
		if(_25d8 != this.workflow.getCurrentSelection()) {
			...
			this.cols[index * 3] = new draw2d.SnapToGeometryEntry(-1, _25d9.getX());
			this.rows[index * 3] = new draw2d.SnapToGeometryEntry(-1, _25d9.getY());
			this.cols[index * 3 + 1] = new draw2d.SnapToGeometryEntry(0, _25d9.x + (_25d9.getWidth() - 1) / 2);
			this.rows[index * 3 + 1] = new draw2d.SnapToGeometryEntry(0, _25d9.y + (_25d9.getHeight() - 1) / 2);
			this.cols[index * 3 + 2] = new draw2d.SnapToGeometryEntry(1, _25d9.getRight() - 1);
			this.rows[index * 3 + 2] = new draw2d.SnapToGeometryEntry(1, _25d9.getBottom() - 1);
			index++;
		}
	}
};
...
draw2d.Port.prototype.createCommand = function(_175e) {
	...
	if(_175e.getPolicy() === draw2d.EditPolicy.CONNECT) {
		if(_175e.source.parentNode.id === _175e.target.parentNode.id) {
			...
			return new draw2d.CommandConnect(_175e.canvas, _175e.source, _175e.target);
		}
	}
	return null;
};
...
draw2d.InputPort.prototype.createCommand = function(_288a) {
	if(_288a.getPolicy() == draw2d.EditPolicy.CONNECT) {
		...
		if(_288a.source instanceof draw2d.OutputPort) {
			return new draw2d.CommandConnect(_288a.canvas, _288a.source, _288a.target);
		}
		return null;
	}
...
draw2d.OutputPort.prototype.createCommand = function(_296d) {
	if(_296d.getPolicy() === draw2d.EditPolicy.CONNECT) {
		...
		if(_296d.source instanceof draw2d.InputPort) {
			return new draw2d.CommandConnect(_296d.canvas, _296d.target, _296d.source);
		}
		return null;
	}
...
draw2d.ManhattanConnectionRouter.prototype._route = function(conn, _17af, _17b0, toPt, toDir) {
	var TOL = 0.1;
	var _17b4 = 0.01;
	var UP = 0;
	var RIGHT = 1;
		...
		if(_17b0 == RIGHT) {
			...
			if(_17b0 == DOWN) {
				if(((xDiff * xDiff) < TOL) && (yDiff < 0) && (toDir == UP)) {
					...
					if(yDiff > 0) {
						point = new draw2d.Point(_17af.x, _17af.y + this.MINDIST);
					} else {
						if(((xDiff > 0) && (toDir === RIGHT)) || ((xDiff < 0) && (toDir === LEFT))) {
							point = new draw2d.Point(_17af.x, toPt.y);
						} else {
							if(_17b0 === toDir) {
								var pos = Math.max(_17af.y, toPt.y) + this.MINDIST;
								point = new draw2d.Point(_17af.x, pos);
							} else {
								point = new draw2d.Point(_17af.x, _17af.y - (yDiff / 2));
							}
						}
					}
					if(xDiff > 0) {
					...
					if(((xDiff * xDiff) < TOL) && (yDiff > 0) && (toDir === DOWN)) {
						...
						if(yDiff < 0) {
							...
							if(((xDiff > 0) && (toDir === RIGHT)) || ((xDiff < 0) && (toDir === LEFT))) {
								...
								if(_17b0 === toDir) {
									var pos = Math.min(_17af.y, toPt.y) - this.MINDIST;
									point = new draw2d.Point(_17af.x, pos);
								} else {
									point = new draw2d.Point(_17af.x, _17af.y - (yDiff / 2));
								}
...
draw2d.BezierConnectionRouter.prototype.drawBezier = function(_26b3, _26b4, t, iter) {
...
draw2d.BezierConnectionRouter.prototype.route = function(conn) {
	...
	var toPt = conn.getEndPoint();
	this._route(_26bf, conn, toPt, this.getEndDirection(conn), _26c0, this.getStartDirection(conn));
	var _26c2 = [];
...
draw2d.BezierConnectionRouter.prototype._route = function(_26c4, conn, _26c6, _26c7, toPt, toDir) {
	var TOL = 0.1;
	var _26cb = 0.01;
	var _26cc = 90;
	var UP = 0;
					...
					if(yDiff > 0) {
						...
						if(((xDiff > 0) && (toDir === RIGHT)) || ((xDiff < 0) && (toDir === LEFT))) {
							...
							if(_26c7 === toDir) {
								...
								point = new draw2d.Point(_26c6.x, _26c6.y - (yDiff / 2));
							}
						}
					}
					if(xDiff > 0) {
...
draw2d.FanConnectionRouter.prototype.route = function(conn) {
	...
	for(var i = 0; i < lines.getSize(); i++) {
		...
		if(_1618.getTarget() == conn.getTarget() || _1618.getSource() == conn.getTarget()) {
			_1615.add(_1618);
			if(conn == _1618) {
				index = _1615.getSize();
			}
...
draw2d.FanConnectionRouter.prototype.routeCollision = function(conn, index) {
	...
	var ray;
	if(_1620 == draw2d.PositionConstants.SOUTH || _1620 == draw2d.PositionConstants.EAST) {
	...
	var _1625;
	if(index % 2 === 0) {
		_1625 = new draw2d.Point(_161f.x + (index / 2) * (-1 * _1624), _161f.y + (index / 2) * _1623);
	} else {
		_1625 = new draw2d.Point(_161f.x + (index / 2) * _1624, _161f.y + (index / 2) * (-1 * _1623));
	}
	conn.addPoint(_1625);
	conn.addPoint(end);
};
...
draw2d.Graphics.prototype.fillPolygon = function(_25ba, _25bb) {
	...
	for(var i = 0; i < _25ba.length; i++) {
		rotX[i] = this.xt + _25ba[i] * this.cosRadian - _25bb[i] * this.sinRadian;
		rotY[i] = this.yt + _25ba[i] * this.sinRadian + _25bb[i] * this.cosRadian;
	}
...
draw2d.Graphics.prototype.drawPolygon = function(_25c0, _25c1) {
	...
	for(var i = 0; i < _25c0.length; i++) {
		rotX[i] = this.xt + _25c0[i] * this.cosRadian - _25c1[i] * this.sinRadian;
		rotY[i] = this.yt + _25c0[i] * this.sinRadian + _25c1[i] * this.cosRadian;
	}
...
draw2d.Connection.prototype.getStartAngle = function() {
	...
	var _20e6 = Math.sqrt((p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y));
...
draw2d.Connection.prototype.getEndAngle = function() {
	...
	var _20ea = Math.sqrt((p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y));
...
draw2d.CanvasDocument.prototype.getFigures = function() {
	...
	for(var i = 0; i < _2c13.getSize(); i++) {
		...
		if(_2c14.indexOf(_2c16) == -1 && _2c16.getParent() === null && !( _2c16 instanceof draw2d.WindowFigure)) {
			_2c12.add(_2c16);
		}
	}
	return _2c12;
...
draw2d.ResizeHandle.prototype.onDragstart = function(x, y) {
	...
	this.commandMove = _2a6b.createCommand(new draw2d.EditPolicy(draw2d.EditPolicy.MOVE));
	this.commandResize = _2a6b.createCommand(new draw2d.EditPolicy(draw2d.EditPolicy.RESIZE));
	return true;
};
draw2d.ResizeHandle.prototype.onDrag = function() {
	...
	switch(this.type) {
		case 1:
			//this.workflow.currentSelection.setPosition(_2a70 - diffX, _2a71 - diffY);
			//this.workflow.currentSelection.setDimension(_2a72 + diffX, _2a73 + diffY);
			break;
		case 2:
			if((diffY > 0 && _2a73 <= 400) || (diffY < 0 && _2a73 >= 155)) {
		...
		case 3:
			...
			break;
		case 4:
			if((diffX < 0 && _2a72 <= 400) || (diffX > 0 && _2a72 >= 125)) {
	...
	try {
		...
		if( _2ba9 instanceof draw2d.Line) {
			...
			if(obj === null) {
				...
				this.html.insertBefore(_2ba9.getHTMLElement(), obj.getHTMLElement());
			}
		}
		if(!_2bac) {
			_2ba9.paint();
...
draw2d.Canvas.prototype.setBackgroundImage = function(_2bb7, _2bb8) {
	if(_2bb7 !== null) {
		if(_2bb8) {
			...
			this.html.style.background = "transparent url(" + _2bb7 + ") no-repeat";
		}
	} else {
	...
	try {
		...
		if(this.html !== null) {
			...
			var _26f3 = function() {
				...
				var line = oThis.getBestLine(diffX + _26f7 - _26f9, diffY + _26f8 - _26fa, null);
				if(line !== null) {
					line.onContextMenu(diffX + _26f7 - _26f9, diffY + _26f8 - _26fa);
				} else {
					oThis.onContextMenu(diffX + _26f7 - _26f9, diffY + _26f8 - _26fa);
				}
			};
			...
			var _2719 = function(e) {
				...
				if(draw2d.Drag.currentHover !== null && obj === null) {
					...
					var _2720 = oThis.getAbsoluteX();
					var _2721 = oThis.getAbsoluteY();
					oThis.onMouseMove(diffX + _271e - _2720, diffY + _271f - _2721);
				}
				if(obj === null) {
					draw2d.Drag.currentHover = null;
				}
			...
			var _2724 = function() {
				...
				if(draw2d.Drag.currentHover !== null && obj === null) {
					...
					var _272a = oThis.getAbsoluteX();
					var _272b = oThis.getAbsoluteY();
					oThis.onMouseMove(diffX + _2728 - _272a, diffY + _2729 - _272b);
				}
				if(obj === null) {
					draw2d.Drag.currentHover = null;
				}
			...
			var _272e = function(_272f) {
				...
				var line = oThis.getBestLine(diffX + _2732 - _2734, diffY + _2733 - _2735, null);
				if(line !== null) {
					line.onDoubleClick();
				}
			};
...
draw2d.Workflow.prototype.clear = function() {
	...
	//var _273a = _2737.getFigures(); // change by Harshal
...
draw2d.Workflow.prototype.addFigure = function(_275f, xPos, yPos) {
	try {
		...
		if( _275f instanceof draw2d.Line) {
			...
			if(this.figures.get(this.figures.size - 1) != _275f) {// change by Harshal (to avoid clone element)
...
draw2d.Workflow.prototype.getBestCompartmentFigure = function(x, y, _276a) {
	...
	for(var i = 0; i < this.figures.getSize(); i++) {
		...
		if(( _276d instanceof draw2d.CompartmentFigure) && _276d.isOver(x, y) == true && _276d != _276a) {
			if(_276b === null) {
				_276b = _276d;
			} else {
...
draw2d.Workflow.prototype.setCurrentSelection = function(_2780) {
	...
	if( _2780 instanceof draw2d.Line) {
		...
		if(!( _2780 instanceof draw2d.Connection)) {
			this.draggingLineCommand = _2780.createCommand(new draw2d.EditPolicy(draw2d.EditPolicy.MOVE));
...
draw2d.Workflow.prototype.showLineResizeHandles = function(_278d) {
	...
	draw2d.Canvas.prototype.addFigure.call(this, this.resizeHandleStart, _2790.x - _278e, _2790.y - _278e);
	draw2d.Canvas.prototype.addFigure.call(this, this.resizeHandleEnd, _2791.x - _278e, _2791.y - _278e);
...
draw2d.Workflow.prototype.showResizeHandles = function(_2792) {
	this.hideLineResizeHandles();
	this.hideResizeHandles();
	if(this.getEnableSmoothFigureHandling() == true && this.getCurrentSelection() != _2792) {
	...
	var xPos = _2792.getX();
	var yPos = _2792.getY();
	draw2d.Canvas.prototype.addFigure.call(this, this.resizeHandle1, xPos - _2793, yPos - _2794);
	draw2d.Canvas.prototype.addFigure.call(this, this.resizeHandle3, xPos + _2796, yPos - _2794);
	draw2d.Canvas.prototype.addFigure.call(this, this.resizeHandle5, xPos + _2796, yPos + _2795);
	draw2d.Canvas.prototype.addFigure.call(this, this.resizeHandle7, xPos - _2793, yPos + _2795);
	...
	if(_2792.isStrechable() && _2792.isResizeable()) {
		...
		draw2d.Canvas.prototype.addFigure.call(this, this.resizeHandle2, xPos + (_2796 / 2) - this.resizeHandleHalfWidth, yPos - _2794);
		draw2d.Canvas.prototype.addFigure.call(this, this.resizeHandle4, xPos + _2796, yPos + (_2795 / 2) - (_2794 / 2));
		draw2d.Canvas.prototype.addFigure.call(this, this.resizeHandle6, xPos + (_2796 / 2) - this.resizeHandleHalfWidth, yPos + _2795);
		draw2d.Canvas.prototype.addFigure.call(this, this.resizeHandle8, xPos - _2793, yPos + (_2795 / 2) - (_2794 / 2));
...
draw2d.Workflow.prototype.moveResizeHandles = function(_279a) {
	...
	if(_279a.isStrechable()) {
		this.resizeHandle2.setPosition(xPos + (_279e / 2) - this.resizeHandleHalfWidth, yPos - _279c);
		this.resizeHandle4.setPosition(xPos + _279e, yPos + (_279d / 2) - (_279c / 2));
		this.resizeHandle6.setPosition(xPos + (_279e / 2) - this.resizeHandleHalfWidth, yPos + _279d);
		this.resizeHandle8.setPosition(xPos - _279b, yPos + (_279d / 2) - (_279c / 2));
	}
};
...
draw2d.Workflow.prototype.snapToHelper = function(_27ae, pos) {
	if(this.snapToGeometryHelper !== null) {
		if( _27ae instanceof draw2d.ResizeHandle) {
			...
			if((_27b2 & draw2d.SnapToHelper.EAST_WEST) && !(_27b3 & draw2d.SnapToHelper.EAST_WEST)) {
				this.showSnapToHelperLineVertical(_27b1.x);
			} else {
				this.hideSnapToHelperLineVertical();
			}
			if((_27b2 & draw2d.SnapToHelper.NORTH_SOUTH) && !(_27b3 & draw2d.SnapToHelper.NORTH_SOUTH)) {
			...
			var _27b4 = new draw2d.Dimension(pos.x, pos.y, _27ae.getWidth(), _27ae.getHeight());
			var _27b1 = new draw2d.Dimension(pos.x, pos.y, _27ae.getWidth(), _27ae.getHeight());
			var _27b2 = draw2d.SnapToHelper.NSEW;
			var _27b3 = this.snapToGeometryHelper.snapRectangle(_27b4, _27b1);
			if((_27b2 & draw2d.SnapToHelper.WEST) && !(_27b3 & draw2d.SnapToHelper.WEST)) {
				this.showSnapToHelperLineVertical(_27b1.x);
			} else {
				if((_27b2 & draw2d.SnapToHelper.EAST) && !(_27b3 & draw2d.SnapToHelper.EAST)) {
					this.showSnapToHelperLineVertical(_27b1.getX() + _27b1.getWidth());
				} else {
					this.hideSnapToHelperLineVertical();
				}
			}
			if((_27b2 & draw2d.SnapToHelper.NORTH) && !(_27b3 & draw2d.SnapToHelper.NORTH)) {
				this.showSnapToHelperLineHorizontal(_27b1.y);
			} else {
				if((_27b2 & draw2d.SnapToHelper.SOUTH) && !(_27b3 & draw2d.SnapToHelper.SOUTH)) {
					this.showSnapToHelperLineHorizontal(_27b1.getY() + _27b1.getHeight());
				} else {
					this.hideSnapToHelperLineHorizontal();
				}
			}
...
draw2d.WindowFigure.prototype.createHTMLElement = function() {
	...
	if(this.hasTitleBar()) {
		...
		this.titlebar.style.font = "bold 10px verdana";
		...
		this.titlebar.style.backgroundImage = "url(assert/images/title.jpeg)";
...
draw2d.LineColorDialog.prototype.onOk = function() {
	...
	if( typeof this.figure.setColor == "function") {
		_2067.getCommandStack().execute(new draw2d.CommandSetColor(this.figure, this.getSelectedColor()));
...
draw2d.BackgroundColorDialog.prototype.onOk = function() {
	...
	if( typeof this.figure.setBackgroundColor == "function") {
		_1d58.getCommandStack().execute(new draw2d.CommandSetBackgroundColor(this.figure, this.getSelectedColor()));
...
draw2d.CommandDelete = function(_200f) {
	...
	this.figure = _200f;
	this.workflow = _200f.workflow//.model.get("workspace"); //chnage by Harshal
		...
		if(li.addEventListener) {
			...
			if(li.attachEvent) {
				li.attachEvent("onclick", function(event) {
					...
					event.srcElement.menuItem.execute(diffX + _177a, diffY + _177b);
				});
...
draw2d.AbstractObjectModel.prototype.firePropertyChange = function(_20ac, _20ad, _20ae) {
...
draw2d.AbstractObjectModel.prototype.removePropertyChangeListener = function(_20b3) {
	if(_20b3 !== null) {
...
XMLP.prototype._checkStructure = function(_28bb) {
	...
	if(XMLP._STATE_MISC == this.m_iState) {
		if((XMLP._ELM_B == _28bb) || (XMLP._ELM_E == _28bb) || (XMLP._ELM_EMP == _28bb) || (XMLP.EVT_DTD == _28bb)) {
			return this._setErr(XMLP.ERR_DOC_STRUCTURE);
		}
		if((XMLP._TEXT == _28bb) || (XMLP._ENTITY == _28bb)) {
			if(SAXStrings.indexOfNonWhitespace(this.getContent(), this.getContentBegin(), this.getContentEnd()) != -1) {
		...
		if(_28ec == XMLP._ELM_B) {
			...
			if(_28ec == XMLP._ELM_E) {
				...
				if(_28ec == XMLP._ELM_EMP) {
					...
					if(_28ec == XMLP._TEXT) {
						this._fireEvent(SAXDriver.CHARS, _28eb.getContent(), _28eb.getContentBegin(), _28eb.getContentEnd() - _28eb.getContentBegin());
					} else {
						if(_28ec == XMLP._ENTITY) {
							this._fireEvent(SAXDriver.CHARS, _28eb.getContent(), _28eb.getContentBegin(), _28eb.getContentEnd() - _28eb.getContentBegin());
						} else {
							if(_28ec == XMLP._PI) {
								this._fireEvent(SAXDriver.PI, _28eb.getName(), _28eb.getContent().substring(_28eb.getContentBegin(), _28eb.getContentEnd()));
							} else {
								if(_28ec == XMLP._CDATA) {
									...
									if(_28ec == XMLP._COMMENT) {
										this._fireEvent(SAXDriver.CMNT, _28eb.getContent(), _28eb.getContentBegin(), _28eb.getContentEnd() - _28eb.getContentBegin());
									} else {
										if(_28ec == XMLP._DTD) {
										} else {
											if(_28ec == XMLP._ERROR) {
												this._fireError(_28eb.getContent());
											} else {
												if(_28ec == XMLP._NONE) {
													return;
												}
...
function trim(_2909, _290a, _290b) {
	...
	var k = 0;
	if(_290a == true) {
		while((i < _2909.length) && (whitespace.indexOf(_2909.charAt(i++)) != -1)) {
			left++;
		}
	}
	if(_290b == true) {
...
DOMImplementation.prototype.hasFeature = function DOMImplementation_hasFeature(_1da5, _1da6) {
	var ret = false;
			...
			if(iEvt == XMLP._ELM_E) {
				...
				if(iEvt == XMLP._ELM_EMP) {
					...
					if(iEvt == XMLP._TEXT || iEvt == XMLP._ENTITY) {
						var _1db7 = p.getContent().substring(p.getContentBegin(), p.getContentEnd());
						...
						if(iEvt == XMLP._PI) {
							...
							if(iEvt == XMLP._CDATA) {
								_1db7 = p.getContent().substring(p.getContentBegin(), p.getContentEnd());
								if(!this.preserveWhiteSpace) {
									_1db7 = trim(_1db7, true, true);
									_1db7.replace(/ +/g, " ");
								}
								if(_1db7.length > 0) {
									iNodeParent.appendChild(doc.createCDATASection(_1db7));
								}
							} else {
								if(iEvt == XMLP._COMMENT) {
									...
									if(_1db7.length > 0) {
										iNodeParent.appendChild(doc.createComment(_1db7));
									}
								} else {
									if(iEvt == XMLP._DTD) {
									} else {
...
DOMImplementation.prototype._isNamespaceDeclaration = function DOMImplementation__isNamespaceDeclaration(_1dc2) {
	return (_1dc2.indexOf("xmlns") > -1);
};
DOMImplementation.prototype._isIdDeclaration = function DOMImplementation__isIdDeclaration(_1dc3) {
...
DOMImplementation.prototype._parseNSName = function DOMImplementation__parseNSName(_1dc6) {
	var _1dc7 = {};
	_1dc7.prefix = _1dc6;
	_1dc7.namespaceName = "";
...
DOMImplementation.prototype._parseQName = function DOMImplementation__parseQName(_1dc8) {
	var _1dc9 = {};
...
DOMNodeList.prototype._insertBefore = function DOMNodeList__insertBefore(_1dd1, _1dd2) {
...
DOMNodeList.prototype._replaceChild = function DOMNodeList__replaceChild(_1dd4, _1dd5) {
	var ret = null;
...
DOMNodeList.prototype._cloneNodes = function DOMNodeList__cloneNodes(deep, _1ddd) {
...
DOMNamedNodeMap.prototype.getNamedItemNS = function DOMNamedNodeMap_getNamedItemNS(_1dee, _1def) {
	var ret = null;
...
DOMNamedNodeMap.prototype.removeNamedItemNS = function DOMNamedNodeMap_removeNamedItemNS(_1df5, _1df6) {
	var ret = null;
	if(this.ownerDocument.implementation.errorChecking && (this._readonly || (this.parentNode && this.parentNode._readonly))) {
		throw (new DOMException(DOMException.NO_MODIFICATION_ALLOWED_ERR));
	}
...
DOMNamedNodeMap.prototype._findNamedItemNSIndex = function DOMNamedNodeMap__findNamedItemNSIndex(_1dfd, _1dfe) {
	var ret = -1;
	if(_1dfe) {
		for(var i = 0; i < this._nodes.length; i++) {
			if((this._nodes[i].namespaceURI == _1dfd) && (this._nodes[i].localName == _1dfe)) {
				ret = i;
				break;
			}
		}
...
DOMNamedNodeMap.prototype._hasAttributeNS = function DOMNamedNodeMap__hasAttributeNS(_1e04, _1e05) {
	var ret = false;
...
DOMNamedNodeMap.prototype._cloneNodes = function DOMNamedNodeMap__cloneNodes(_1e08) {
...
DOMNamespaceNodeMap.prototype._findNamedItemIndex = function DOMNamespaceNodeMap__findNamedItemIndex(_1e0f) {
	var ret = -1;
	for(var i = 0; i < this._nodes.length; i++) {
...
DOMNamespaceNodeMap.prototype._cloneNodes = function DOMNamespaceNodeMap__cloneNodes(_1e12) {
...
DOMNode.prototype.setPrefix = function DOMNode_setPrefix(_1e1a) {
	if(this.ownerDocument.implementation.errorChecking) {
		...
		if(!this.ownerDocument._isValidNamespace(this.namespaceURI, _1e1a + ":" + this.localName)) {
			throw (new DOMException(DOMException.NAMESPACE_ERR));
		}
		if((_1e1a == "xmlns") && (this.namespaceURI != "http://www.w3.org/2000/xmlns/")) {
			throw (new DOMException(DOMException.NAMESPACE_ERR));
		}
...
DOMNode.prototype.insertBefore = function DOMNode_insertBefore(_1e1b, _1e1c) {
	...
	if(_1e1c) {
		...
		this.childNodes._insertBefore(_1e1b, this.childNodes._findItemIndex(_1e1c._id));
...
DOMNode.prototype.removeChild = function DOMNode_removeChild(_1e27) {
	if(this.ownerDocument.implementation.errorChecking && (this._readonly || _1e27._readonly)) {
		throw (new DOMException(DOMException.NO_MODIFICATION_ALLOWED_ERR));
	}
...
DOMNode.prototype.getElementsByTagName = function DOMNode_getElementsByTagName(_1e33) {
	return this._getElementsByTagNameRecursive(_1e33, new DOMNodeList(this.ownerDocument));
};
DOMNode.prototype._getElementsByTagNameRecursive = function DOMNode__getElementsByTagNameRecursive(_1e34, _1e35) {
	if(this.nodeType == DOMNode.ELEMENT_NODE || this.nodeType == DOMNode.DOCUMENT_NODE) {
		if((this.nodeName == _1e34) || (_1e34 == "*")) {
			_1e35._appendChild(this);
		}
		for(var i = 0; i < this.childNodes.length; i++) {
			_1e35 = this.childNodes.item(i)._getElementsByTagNameRecursive(_1e34, _1e35);
		}
	}
	return _1e35;
};
...
DOMNode.prototype.getElementsByTagNameNS = function DOMNode_getElementsByTagNameNS(_1e37, _1e38) {
	return this._getElementsByTagNameNSRecursive(_1e37, _1e38, new DOMNodeList(this.ownerDocument));
};
DOMNode.prototype._getElementsByTagNameNSRecursive = function DOMNode__getElementsByTagNameNSRecursive(_1e39, _1e3a, _1e3b) {
	if(this.nodeType == DOMNode.ELEMENT_NODE || this.nodeType == DOMNode.DOCUMENT_NODE) {
		if(((this.namespaceURI == _1e39) || (_1e39 == "*")) && ((this.localName == _1e3a) || (_1e3a == "*"))) {
			_1e3b._appendChild(this);
		}
		for(var i = 0; i < this.childNodes.length; i++) {
			_1e3b = this.childNodes.item(i)._getElementsByTagNameNSRecursive(_1e39, _1e3a, _1e3b);
		}
	}
	return _1e3b;
};
	...
	try {
		if(_1e3e.nodeType == DOMNode.ELEMENT_NODE) {
			if(!this.ownerDocument.implementation.namespaceAware) {
				...
				for(var i = 0; i < _1e3e.attributes.length; i++) {
					_1e40.setAttribute(_1e3e.attributes.item(i).name, _1e3e.attributes.item(i).value);
				}
			} else {
				_1e40 = this.ownerDocument.createElementNS(_1e3e.namespaceURI, _1e3e.nodeName);
				for(var i = 0; i < _1e3e.attributes.length; i++) {
					_1e40.setAttributeNS(_1e3e.attributes.item(i).namespaceURI, _1e3e.attributes.item(i).name, _1e3e.attributes.item(i).value);
				}
				for(var i = 0; i < _1e3e._namespaces.length; i++) {
					_1e40._namespaces._nodes[i] = this.ownerDocument.createNamespace(_1e3e._namespaces.item(i).localName);
					_1e40._namespaces._nodes[i].setValue(_1e3e._namespaces.item(i).value);
				}
			}
		} else {
			if(_1e3e.nodeType == DOMNode.ATTRIBUTE_NODE) {
				if(!this.ownerDocument.implementation.namespaceAware) {
					...
					_1e40 = this.ownerDocument.createAttributeNS(_1e3e.namespaceURI, _1e3e.nodeName);
					for(var i = 0; i < _1e3e._namespaces.length; i++) {
						_1e40._namespaces._nodes[i] = this.ownerDocument.createNamespace(_1e3e._namespaces.item(i).localName);
						_1e40._namespaces._nodes[i].setValue(_1e3e._namespaces.item(i).value);
					}
				...
				if(_1e3e.nodeType == DOMNode.DOCUMENT_FRAGMENT) {
					...
					if(_1e3e.nodeType == DOMNode.NAMESPACE_NODE) {
						_1e40 = this.ownerDocument.createNamespace(_1e3e.nodeName);
						_1e40.setValue(_1e3e.value);
					} else {
						if(_1e3e.nodeType == DOMNode.TEXT_NODE) {
							_1e40 = this.ownerDocument.createTextNode(_1e3e.data);
						} else {
							if(_1e3e.nodeType == DOMNode.CDATA_SECTION_NODE) {
								_1e40 = this.ownerDocument.createCDATASection(_1e3e.data);
							} else {
								if(_1e3e.nodeType == DOMNode.PROCESSING_INSTRUCTION_NODE) {
									_1e40 = this.ownerDocument.createProcessingInstruction(_1e3e.target, _1e3e.data);
								} else {
									if(_1e3e.nodeType == DOMNode.COMMENT_NODE) {
										_1e40 = this.ownerDocument.createComment(_1e3e.data);
									} else {
		...
		if(deep) {
			for(var i = 0; i < _1e3e.childNodes.length; i++) {
				_1e40.appendChild(this.ownerDocument.importNode(_1e3e.childNodes.item(i), true));
			}
		}
...
DOMDocument.prototype.createElement = function DOMDocument_createElement(_1e45) {
	if(this.ownerDocument.implementation.errorChecking && (!this.ownerDocument.implementation._isValidName(_1e45))) {
		throw (new DOMException(DOMException.INVALID_CHARACTER_ERR));
	}
...
DOMDocument.prototype.createProcessingInstruction = function DOMDocument_createProcessingInstruction(_1e4e, data) {
	if(this.ownerDocument.implementation.errorChecking && (!this.implementation._isValidName(_1e4e))) {
		throw (new DOMException(DOMException.INVALID_CHARACTER_ERR));
	}
...
DOMDocument.prototype.createElementNS = function DOMDocument_createElementNS(_1e53, _1e54) {
	if(this.ownerDocument.implementation.errorChecking) {
...
DOMDocument.prototype.createAttributeNS = function DOMDocument_createAttributeNS(_1e57, _1e58) {
	if(this.ownerDocument.implementation.errorChecking) {
...
DOMDocument.prototype.createNamespace = function DOMDocument_createNamespace(_1e5b) {
...
DOMDocument.prototype.getElementById = function DOMDocument_getElementById(_1e5e) {
	retNode = null;
	for(var i = 0; i < this.all.length; i++) {
		var node = this.all[i];
		if((node.id == _1e5e) && (node._isAncestor(node.ownerDocument.documentElement))) {
			retNode = node;
			break;
		}
	}
...
DOMDocument.prototype._isValidNamespace = function DOMDocument__isValidNamespace(_1e61, _1e62, _1e63) {
	...
	if((valid) && (qName.prefix == "xml") && (_1e61 != "http://www.w3.org/XML/1998/namespace")) {
		valid = false;
	}
	return valid;
};
...
DOMElement.prototype.setAttributeNode = function DOMElement_setAttributeNode(_1e6f) {
	if(this.ownerDocument.implementation._isIdDeclaration(_1e6f.name)) {
		this.id = _1e6f.value;
	}
	return this.attributes.setNamedItem(_1e6f);
};
DOMElement.prototype.removeAttributeNode = function DOMElement_removeAttributeNode(_1e70) {
...
DOMElement.prototype.getAttributeNS = function DOMElement_getAttributeNS(_1e72, _1e73) {
	var ret = "";
...
DOMElement.prototype.setAttributeNS = function DOMElement_setAttributeNS(_1e76, _1e77, value) {
...
DOMElement.prototype.removeAttributeNS = function DOMElement_removeAttributeNS(_1e7a, _1e7b) {
	return this.attributes.removeNamedItemNS(_1e7a, _1e7b);
};
DOMElement.prototype.getAttributeNodeNS = function DOMElement_getAttributeNodeNS(_1e7c, _1e7d) {
	return this.attributes.getNamedItemNS(_1e7c, _1e7d);
};
DOMElement.prototype.setAttributeNodeNS = function DOMElement_setAttributeNodeNS(_1e7e) {
	if((_1e7e.prefix == "") && this.ownerDocument.implementation._isIdDeclaration(_1e7e.name)) {
		this.id = _1e7e.value;
	}
...
DOMElement.prototype.hasAttributeNS = function DOMElement_hasAttributeNS(_1e80, _1e81) {
...
DOMCharacterData.prototype.substringData = function DOMCharacterData_substringData(_1e8f, count) {
	var ret = null;
	if(this.data) {
		if(this.ownerDocument.implementation.errorChecking && ((_1e8f < 0) || (_1e8f > this.data.length) || (count < 0))) {
...
DOMCharacterData.prototype.insertData = function DOMCharacterData_insertData(_1e93, arg) {
	if(this.ownerDocument.implementation.errorChecking && this._readonly) {
		throw (new DOMException(DOMException.NO_MODIFICATION_ALLOWED_ERR));
	}
	if(this.data) {
		if(this.ownerDocument.implementation.errorChecking && ((_1e93 < 0) || (_1e93 > this.data.length))) {
			throw (new DOMException(DOMException.INDEX_SIZE_ERR));
		}
		this.setData(this.data.substring(0, _1e93).concat(arg, this.data.substring(_1e93)));
	} else {
...
DOMCharacterData.prototype.deleteData = function DOMCharacterData_deleteData(_1e95, count) {
	if(this.ownerDocument.implementation.errorChecking && this._readonly) {
		throw (new DOMException(DOMException.NO_MODIFICATION_ALLOWED_ERR));
	}
	if(this.data) {
		if(this.ownerDocument.implementation.errorChecking && ((_1e95 < 0) || (_1e95 > this.data.length) || (count < 0))) {
			throw (new DOMException(DOMException.INDEX_SIZE_ERR));
		}
		if(!count || (_1e95 + count) > this.data.length) {
			this.setData(this.data.substring(0, _1e95));
		} else {
			this.setData(this.data.substring(0, _1e95).concat(this.data.substring(_1e95 + count)));
		}
	}
};
DOMCharacterData.prototype.replaceData = function DOMCharacterData_replaceData(_1e97, count, arg) {
	if(this.ownerDocument.implementation.errorChecking && this._readonly) {
		throw (new DOMException(DOMException.NO_MODIFICATION_ALLOWED_ERR));
	}
	if(this.data) {
		if(this.ownerDocument.implementation.errorChecking && ((_1e97 < 0) || (_1e97 > this.data.length) || (count < 0))) {
			throw (new DOMException(DOMException.INDEX_SIZE_ERR));
		}
		this.setData(this.data.substring(0, _1e97).concat(arg, this.data.substring(_1e97 + count)));
	} else {
		this.setData(arg);
	}
};
...
draw2d.XMLDeserializer.fromXML = function(node, _1fec) {
	...
	for(var i = 0; i < _1ff0.length; i++) {
		...
		obj[_1ff3] = draw2d.XMLDeserializer.fromXML(child, obj instanceof draw2d.AbstractObjectModel ? obj : _1fec);
	}
	return obj;
};
